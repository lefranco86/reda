...
export type ProvinceState = Readonly<typeof initialState>;

// Reducer

export default (state: ProvinceState = initialState, action): ProvinceState => {
  switch (action.type) {
...
export const getEntities: ICrudGetAllAction<IProvince> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_PROVINCE_LIST,
  payload: axios.get<IProvince>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});

export const getEntity: ICrudGetAction<IProvince> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_PROVINCE,
    payload: axios.get<IProvince>(requestUrl)
  };
};

export const createEntity: ICrudPutAction<IProvince> = entity => async dispatch => {
...
export const updateEntity: ICrudPutAction<IProvince> = entity => async dispatch => {
  const result = await dispatch({
...
export const deleteEntity: ICrudDeleteAction<IProvince> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
